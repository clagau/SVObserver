...
void SVOCVAnalyzerResult::HideResults()
{
	if ( !m_bHasLicenseError )
	{
		...
		for( long l = 0; l < OCV_MAX_RESULTS; l++ )
		{
			SVOCVCharacterResult *pResult = dynamic_cast<SVOCVCharacterResult*>(getTaskObject( l ));
			pResult->SetObjectAttributesAllowed( SvPb::useable, SvOi::SetAttributeType::OverwriteAttribute );
	...
	if( bOk && !rRunStatus.IsDisabled() && !rRunStatus.IsDisabledByCondition() )
	{
		...
		else
		{
			...
			if( nullptr != pInputBuffer && ! pInputBuffer->isEmpty())
			{
				...
				if( nullptr == pMilBuffer )
				{
					...
					if (nullptr != pErrorMessages)
					{
						SvStl::MessageContainer Msg( SVMSG_SVO_93_GENERAL_WARNING, SvStl::Tid_Error_NoMilHostBuffer, SvStl::SourceFileParams(StdMessageParams), getObjectId());
						pErrorMessages->push_back(Msg);
					}
					bOk = false;
				}
		...
		if( bOk )
		{
			...
			if( !lock.Lock( 1 ) )
			{
				...
				if (nullptr != pErrorMessages)
				{
					SvStl::MessageContainer Msg( SVMSG_SVO_92_GENERAL_ERROR, SvStl::Tid_LockingFailed, SvStl::SourceFileParams(StdMessageParams), getObjectId());
					pErrorMessages->push_back(Msg);
				}
			}
			else
			{
				...
				// Preset for failure
				//
				m_pwf.setWarnedFailedStatus(true, true);
				...
				try
				{
					while (1)
					{
						...
						if( bOperation ) // read
						{
							...
							strFunctionName = _T("MocrReadString");
						...
						{ // verify
							...
							strFunctionName = _T("MocrVerifyString");
...
//-						A string was found!
				...
				catch( ... ) // Access violation in : 5eec0a51 milpat!MpatFindMultipleModelUnpack
				{
					...
					if (nullptr != pErrorMessages)
					{
						...
						SvStl::MessageContainer Msg( SVMSG_SVO_30_EXCEPTION_IN_MIL, SvStl::Tid_Default, msgList, SvStl::SourceFileParams(StdMessageParams), getObjectId());
						pErrorMessages->push_back(Msg);
					}
					bOk = false;
					l_lLength = 0;
...
//-				Set unused indexes.
				...
				switch ( nResultValue )
				{
					...
					case 2: // Warned
					{
						m_pwf.m_Failed.SetValue(BOOL(false));
						m_pwf.m_Warned.SetValue(BOOL(true));
	...
	if (MatroxCode & SV_ARC_ERROR)
	{
		...
		if (nullptr != pErrorMessages)
		{
			SvStl::MessageContainer Msg( static_cast<DWORD> (MatroxCode), SvStl::Tid_ErrorIn, msgList, SvStl::SourceFileParams(StdMessageParams), getObjectId());
			pErrorMessages->push_back(Msg);
		}
		bOk = false;
	}
