...
namespace
{
bool isChangable(const ValueController& rValueController, SvPb::EmbeddedIdEnum embeddedId)
{
	auto data = rValueController.Get<LinkedValueData>(embeddedId);
	return SvPb::LinkedSelectedOption::DirectValue == data.m_selectedOption;
}

std::vector<double> createPointPairVec(SvPb::EmbeddedIdEnum xEid, SvPb::EmbeddedIdEnum yEid, const ValueController& rControl)
{
...
void setPointPairsToValueController(const variant_t& rValue, SvPb::EmbeddedIdEnum xEid, SvPb::EmbeddedIdEnum yEid, ValueController& rControl)
{
...
void setArrayToValueController(const variant_t& rValue, SvPb::EmbeddedIdEnum embeddedId, ValueController& rControl)
{
	if (VT_ARRAY == (VT_ARRAY & rValue.vt) && nullptr != rValue.parray)
	{
	...
	switch (type)
	{
		...
		case DrawNodeType::Segment:
		{
			...
			if (isXAxisAngleUsed)
			{
				...
				xyVec.push_back(rValueController.Get<LinkedValueData>(SvPb::CenterXEId).m_Value);
				xyVec.push_back(rValueController.Get<LinkedValueData>(SvPb::CenterYEId).m_Value);
		...
		case DrawNodeType::BucketFill:
		{
			...
			parMap[CDSVPictureDisplay::P_SubType] = CDSVPictureDisplay::SubType_Points;
			std::vector<double> xyVec;
			xyVec.push_back(rValueController.Get<LinkedValueData>(SvPb::X1EId).m_Value);
			xyVec.push_back(rValueController.Get<LinkedValueData>(SvPb::Y1EId).m_Value);
...
void setOverlayProperties(DrawNodeType type, ValueController& rValueController, VARIANT* ParameterList, VARIANT* ParameterValue)
{
	...
	DisplayHelper::FillParameterMap(ParaMap, ParameterList, ParameterValue);
