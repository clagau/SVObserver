...
namespace SvOgu
{
	...
	public:
		...
		LinkedValueWidgetHelper(CEdit& rValueEdit, CButton& rSelectButton, uint32_t inspectionId, uint32_t taskId, SvPb::EmbeddedIdEnum embeddedId, ValueController* pValueController, ValidCheckCallback validCallback = nullptr, ConvertValueCallback convertCallback = nullptr);

		void setValueController(ValueController* pValueController);
	...
	public:
		...
		{
			return std::make_unique<LinkedValueWidgetHelper>(rValueEdit, rSelectButton, m_inspectionId, m_taskId, embeddedId, m_pValueController, validCallback, convertCallback);
		}
	};
