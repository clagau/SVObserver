...
void CLabel::OnPaint()
{
	...
	if(strText.Find(_T('\t')) != -1) { dwFlags |= DT_EXPANDTABS; }
...
CLabel& CLabel::SetBkColor(COLORREF crBkgnd, COLORREF crBkgndHigh, BackFillMode mode)
{
	...
	if (m_fillmode == Normal) { m_hBackBrush = ::CreateSolidBrush(crBkgnd); }
...
{
	// Arrrrh...
	// You're looking in here for why the font isn't rotating, aren't you?
...
void CLabel::DrawGradientFill(CDC* pDC, CRect* pRect, COLORREF crStart, COLORREF crEnd, int nSegments)
{
	...
	if(nSegments > pRect->Width()) { nSegments = pRect->Width(); }
