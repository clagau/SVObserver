...
public:
	GDICanvas(CWnd * owner, const CRect & client, CBitmap * bmp, COLORREF color)
	{
		...
		m_pOriginalBmp = m_memDC.SelectObject(bmp);
		m_pOriginalBrush = m_memDC.SelectObject(&l_brush);
		m_memDC.FillRect(&client, &l_brush);
	}
	~GDICanvas()
	{
		m_memDC.SelectObject(m_pOriginalBrush);
		m_memDC.SelectObject(m_pOriginalBmp);
	}

	void Draw(const RECT& rRect, int color)
	{
		CBrush l_brush(RGB(color, color, color));
		m_memDC.SelectObject(&l_brush);
		m_memDC.FillRect(&rRect, &l_brush);
	}
private:
	CBitmap* m_pOriginalBmp {nullptr};
	CBrush* m_pOriginalBrush {nullptr};
	CDC m_memDC;
};

SVHistogram::SVHistogram() : CStatic(),
	SVHistogramBase(),
...
void SVHistogram::OnPaint()
{
	...
	CBitmap* pOiginalBmp = memDC.SelectObject(&m_bmp);
	...
	memDC.SelectObject(pOiginalBmp);
	...
	CFont* pOriginalFont = dc.SelectObject(&l_font);
	DrawLabels(dc, this);
