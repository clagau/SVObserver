...
bool TableSortAnalyzer::ResetObject(SvStl::MessageContainerVector *pErrorMessages)
{
		...
		|| nullptr == pColumnValues->GetParent() || pColumnValues->GetParent()->GetParent() != GetParent())
	{
		...
		if (nullptr != pErrorMessages)
		{
			...
			message.setMessage( SVMSG_SVO_92_GENERAL_ERROR, SvStl::Tid_NoValidColumnConnected, SvStl::SourceFileParams(StdMessageParams), getObjectId() );
...
{
	RegisterEmbeddedObject( &m_isASC, SvPb::TableAnaylzerSortIsASCEId, IDS_OBJECTNAME_TABLEANALYZER_ISASC, true, SvOi::SVResetItemToolAndDependent, true);
