...
ClientService::ClientService(SVRPC::RPCClient& rpc_client, boost::posix_time::time_duration request_timeout)
	: m_request_timeout(request_timeout), m_get_version_client(rpc_client), m_query_list_name_client(rpc_client),
	m_query_list_item_client(rpc_client), m_get_product_client(rpc_client), 
	m_get_image_from_cur_id_client(rpc_client), m_get_image_stream_from_cur_id_client(rpc_client),
	m_get_fail_status_client(rpc_client), m_get_items_client(rpc_client), m_set_mode_client(rpc_client)
{
}

void ClientService::getVersion(GetVersionRequest&& req, SVRPC::Task<GetVersionResponse> task)
{
	m_get_version_client.request(std::move(req), task);
}

void ClientService::queryListName(QueryListNameRequest&& req, SVRPC::Task<QueryListNameResponse> task)
{
	m_query_list_name_client.request(std::move(req), task);
}

void ClientService::queryListItem(QueryListItemRequest&& req, SVRPC::Task<QueryListItemResponse> task)
{
	m_query_list_item_client.request(std::move(req), task);
}

void ClientService::getProduct(GetProductRequest&& req, SVRPC::Task<GetProductResponse> task)
{
	m_get_product_client.request(std::move(req), task);
}


void ClientService::getImageFromCurId(GetImageFromCurIdRequest&& req, SVRPC::Task<GetImageFromCurIdResponse> task)
{
	m_get_image_from_cur_id_client.request(std::move(req), task);
}
