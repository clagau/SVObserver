...
void SVImageView::SaveViewOrImageToDisk(bool ViewOnly, bool showOverlays)
{
	...
	if (nullptr == pCurrentImage)
	{
		...
		NullImageWarning.setMessage(SVMSG_SVO_5018_NULLIMAGE, SvStl::Tid_DisplayedImageIsUnavailable, SvStl::SourceFileParams(StdMessageParams));
		return;
	}
		...
		TheSVObserverApp().OkToEdit())
	{
		if (false == ImageIsEmpty())
		{
			...
			if (m_isPicked)
			{
				...
				auto  pToolExtenclass = getToolExtentPtr();
				POINT l_tempPoint;
				...
				if (pToolExtenclass &&
					S_OK == pToolExtenclass->GetExtentProperty(SvPb::SVExtentPropertyPositionPoint, l_tempPoint) &&
...
HICON SVImageView::GetObjectCursor(SvPb::SVExtentLocationPropertyEnum svLocation, POINT point)
{
	...
	if (pToolExtenclass)
	{
		...
		if (SvPb::SVExtentTranslationProfile == pToolExtenclass->GetTranslation())
		{
			if (S_OK == pToolExtenclass->GetExtentProperty(SvPb::SVExtentPropertyRotationAngle, l_dRotationAngle))
			{
				l_dRotationAngle = fmod(l_dRotationAngle, 180.0);
			}
		}
		//Check that this is the Polar Unwrap tool with rotation
		else if (SvPb::SVExtentTranslationPolarUnwrap == pToolExtenclass->GetTranslation())
		{
			SVPoint<double> center;
			SVPoint<double> mousePosition(point);
			if (S_OK == pToolExtenclass->GetExtentProperty(SvPb::SVExtentPropertyPositionPoint, center))
			{
				//Polar unwrap needs an offset of 90° for the displayed cursor to be correct
				l_dRotationAngle = SvUl::SVGetRotationAngle(center, mousePosition) + 90.0;
				l_dRotationAngle = fmod(l_dRotationAngle, 180.0);
			}
		}
