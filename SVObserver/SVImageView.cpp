...
//* .Check In Date   : $Date:   17 Jul 2014 19:11:02  $
//******************************************************************************
...
void SVImageViewClass::Initialize()
{
	...
	m_scaleCount = 31;
	m_scaleIndex = m_scaleCount / 2;

	for (int i = m_scaleCount - 1; 0 <= i; i--)
	{
		// map the first 14 fonts (0 - 14) to the default since they're too small
		// to read anyway.
		if( i < static_cast< int >( m_scaleCount / 2 ) )
		{
			m_scaleFactor[ i ] = 1.0 / ( double )( ( m_scaleCount / 2 + 1 ) - i );

			m_hFont[ i ] = m_hFont[ m_scaleCount / 2 ];
		}
		else
		{
			m_scaleFactor[ i ] = ( double )( i - ( m_scaleCount / 2 - 1 ) );
...
SVImageViewClass::~SVImageViewClass()
{
	//m_ThreadWait.Destroy();

	DetachFromImage();

	// map remaining fonts.
	for( unsigned long i = 0; i < m_scaleCount; i++ )
	{
		if( m_scaleCount / 2 <= i )
		{
...
void SVImageViewClass::GetImageRect( CRect& p_rect )
{
	...
	if( l_pIPDoc != NULL && l_pIPDoc->m_bAllowRefresh )
	{
		...
		p_rect.right = ( long )( ( double )p_rect.right * m_scaleFactor[ m_scaleIndex ] );
		p_rect.bottom = ( long )( ( double )p_rect.bottom * m_scaleFactor[ m_scaleIndex ] );
	}
}
...
BOOL SVImageViewClass::OnCommand( WPARAM p_wParam, LPARAM p_lParam )
{
	...
	switch( p_wParam )
	{
		...
		case ID_ZOOM_SMALL:
		{
			l_index = m_scaleCount / 2 - 3;

			SetScaleIndex( l_index );
			break;
		}

		case ID_ZOOM_NORMAL:
		{
			l_index = m_scaleCount / 2;
		...
		{
			SetScaleIndex( m_scaleCount - 1 );
			break;
		}

		case ID_ZOOM_IN:
		{
			l_index = m_scaleIndex + 1;
...
{
	SVImageClass* l_pImage = dynamic_cast< SVImageClass* >( SVObjectManagerClass::Instance().GetObject( m_ImageId.ToGUID() ) );
	SVDrawContext l_svDrawContext( NULL, l_pImage, m_scaleFactor[ m_scaleIndex ] );
...
bool SVImageViewClass::SetScaleIndex( unsigned long p_scaleIndex )
{
	bool l_bOk = p_scaleIndex < m_scaleCount;

	if( l_bOk )
	{
		if( m_scaleIndex != p_scaleIndex )
		{
			CRect l_rect;

			m_scaleIndex = p_scaleIndex;
...
void SVImageViewClass::UpdateOverlays( HDC p_hDC, long p_X, long p_Y )
{
	...
	if( l_pImage != NULL )
	{
		...
		// Select Font...
		HGDIOBJ hFontOld = ::SelectObject( p_hDC, m_hFont[ m_scaleIndex ] );

		// Use Drawing Context
		SVDrawContext drawContext( p_hDC, l_pImage, m_scaleFactor[ m_scaleIndex ] );
...
BOOL SVImageViewClass::GetParameters(SVObjectWriter& rWriter)
{
	...
	long l_index = m_scaleIndex - m_scaleCount / 2;
...
BOOL SVImageViewClass::SetParameters( SVTreeType& p_tree, SVTreeType::SVBranchHandle p_parent )
{
	...
	if( l_bOk )
	{
		...
		if( l_bOk )
		{
			...
			if( !SetScaleIndex( m_scaleCount / 2 + l_index ) )
			{
				SetScaleIndex( m_scaleCount / 2 );
			}
		}
	}
...
BOOL SVImageViewClass::CheckParameters( SVTreeType& p_tree, SVTreeType::SVBranchHandle p_parent )
{
	...
	if( l_bOk )
	{
		...
		if( l_bOk )
		{
			...
			if( m_scaleIndex != m_scaleCount / 2 + l_index )
			{
				if( !SetScaleIndex( m_scaleCount / 2 + l_index ) )
				{
					SetScaleIndex( m_scaleCount / 2 );
...
HRESULT SVImageViewClass::GetRectInfo( CRect& p_rSourceRect, CRect& p_rDestRect )
{
	...
	if( !( l_BitmapInfo.empty() ) && ( l_pParent != NULL ) )
	{
		...
		double l_zoomFactor = 1 / m_scaleFactor[ m_scaleIndex ];
