...
BEGIN_MESSAGE_MAP(SVIPDoc, CDocument)
	...
	ON_COMMAND(ID_ADD_ROUNDWINDOWTOOL, OnAddRoundWindowTool)
	ON_COMMAND(ID_ADD_ROTATETOOL, OnAddRotateTool)
	...
	ON_UPDATE_COMMAND_UI_RANGE( ID_VIEW_TOOLSETDRAW_POP_BASE + 1, ID_VIEW_TOOLSETDRAW_POP_MAX, OnUpdateViewToolSetDrawSubMenues )
...
//	:15.02.2000 RO			Introduced Freeze Set.
////////////////////////////////////////////////////////////////////////////////
BOOL SVIPDoc::InitAfterSystemIsDocked()
{
	// Init Utility and Tool Set Draw menues...
	InitMenu ();
	
	SVInspectionProcess* l_pInspection( GetInspectionProcess() );

	BOOL l_bOk = ( l_pInspection != NULL );
	
	if( l_bOk )
	{
		BOOL rc = TRUE;
		SVImageViewClass* pView = NULL;
		int i;
		...
		if (IsNew)
		{
			...
			if( pView != NULL )
			{
				...
				if( GetToolSet()->getCurrentImage() != NULL )
				{
					l_ImageId = GetToolSet()->getCurrentImage()->GetUniqueObjectID();
				}

				pView->AttachToImage( l_ImageId );
				//pView->AttachToImage( GetToolSet()->getCurrentImage() );

				SVImageViewScroll* pScroll( NULL );
				...
				if( pScroll != NULL )
				{
					...
					SVIPSplitterFrame* pSplitterFrame( NULL );

					do
					{
						pSplitterFrame = dynamic_cast< SVIPSplitterFrame* >( pWnd );
	
						if( pSplitterFrame == NULL )
						{
							pWnd = pWnd->GetParent();
						}
					}
					while( pWnd != NULL && pSplitterFrame == NULL );
	...
	/*
	SVSVIMStateClass l_oState;

	l_oState.AddState( SV_STATE_INTERNAL_RUN );

	UpdateAllViews( NULL, SVIPDoc::RefreshView );

	l_oState.RemoveState( SV_STATE_INTERNAL_RUN );
	*/
...
BOOL SVIPDoc::AddTool( SVToolClass* PTool )
{
	if( PTool != NULL )
	{
		SVToolClass* pNextTool = NULL;
		int index = 0;
		int count = 0;
		
		if( GetToolSetTabView() )
		{
			index = GetToolSetTabView()->GetCurSel();
			count = GetToolSetTabView()->GetItemCount() - 1; // insert at end

			if( index < 0 )
				index = count; // insert at end

			if( index < count )
				pNextTool = GetToolSetTabView()->GetToolAt( index );
		}
		...
		if( index == 0 )
		{
			...
			{
				AfxMessageBox( "A Color Tool must ALWAYS be\n the first tool on a color system.", MB_OK );
				return FALSE;
			}// end if
		}// end if
		...
		if( PTool != NULL )
		{
			SVAddTool l_AddTool( PTool, index );

			if( SVObjectManagerClass::Instance().UpdateObserver( GetInspectionID(), l_AddTool ) == S_OK )
			{
				SetSelectedToolID( PTool->GetUniqueObjectID() );
...
{
	SVToolSetTabViewClass* l_pView( NULL );

	POSITION pos( GetFirstViewPosition() );

	CView* pIPView( NULL );

	while( l_pView == NULL && pos != NULL )
	{
		pIPView = GetNextView( pos );

		if( pIPView != NULL && pIPView->IsKindOf( RUNTIME_CLASS( SVToolSetTabViewClass ) ) )
		{
...
SVImageViewClass* SVIPDoc::GetImageView( int p_Index )
{
	...
	while( l_pView == NULL && pos != NULL )
	{
		...
		if( pIPView != NULL && pIPView->IsKindOf( RUNTIME_CLASS( SVImageViewClass ) ) )
		{
			if( l_Count == p_Index )
			{
...
{
	SVResultViewClass* l_pView( NULL );
	...
	while( l_pView == NULL && pos != NULL )
	{
		...
		if( pIPView != NULL && pIPView->IsKindOf( RUNTIME_CLASS( SVResultViewClass ) ) )
		{
...
{
	SVInspectionProcess* l_pInspect = NULL;
...
{
	SVConditionalClass* l_pObject( NULL );
...
////////////////////////////////////////////////////////////////////////////////
// .Title       : OnUpdateRunRegressionTest
// -----------------------------------------------------------------------------
// .Description : ...
//              :
////////////////////////////////////////////////////////////////////////////////
...
// .History
//	 Date		Author		Comment                                       
//  :02.10.1997 RO			First Implementation
////////////////////////////////////////////////////////////////////////////////
void SVIPDoc::OnAdjustLightReference() 
{
	...
	if( l_pInspection == NULL || !GetToolSet()->IsCreated() )
	{
		return;
	}
...
//!!		SVLightReferenceDialogPropertySheetClass dlg( CString( _T( "Light Reference Adjustment - " ) ) + pCurrentSystem->GetBoardType() + _T( " - [" ) + GetTitle() + _T( "]" ) );
...
//	:
////////////////////////////////////////////////////////////////////////////////
void SVIPDoc::OnAddReferenceTool() 
{
/*	SVToolClass* pTool = new SVReferenceToolClass;
...
{
	//SVToolClass* pTool = ( SVToolClass* ) TheClassRegister.ConstructNewObject( SVWindowToolClass::SVClassIDSVWindowToolClass );
...
// .History
//	 Date		Author		Comment                                       
//  :24.01.2000 RO			First Implementation
////////////////////////////////////////////////////////////////////////////////
...
// .History
//	 Date		Author		Comment                                       
//  :27.05.1999 FRB			First Implementation
////////////////////////////////////////////////////////////////////////////////
...
// .History
//	 Date		Author		Comment                                       
//  :22.09.1998 RO			First Implementation
////////////////////////////////////////////////////////////////////////////////
void SVIPDoc::OnAddRoundWindowTool() 
{
/*	SVToolClass* pTool = new SVRoundWindowToolClass;
...
//  :
////////////////////////////////////////////////////////////////////////////////
void SVIPDoc::OnAddRotateTool() 
{
/*	SVToolClass* pTool = new SVRotateToolClass;
...
{
	//SVToolClass* pTool = ( SVToolClass* ) TheClassRegister.ConstructNewObject( SVLoadImageToolClassGuid );
...
// .History
//	 Date		Author		Comment                                       
//  :08.02.2000 SEJ			First Implementation
////////////////////////////////////////////////////////////////////////////////
...
// .History
//	 Date		Author		Comment                                       
//  :27.02.2000 RO			First Implementation
//	:23.03.2000 RO			Moved call to set default formulas into AddTool(...)
////////////////////////////////////////////////////////////////////////////////
void SVIPDoc::OnAddPolarUnwrapTool() 
{
	...
	if( AddTool( pTool ) )
	{
		// DONE IN SVIPDoc::AddTool(...) !!!!
		// RO_23Mar2000.
		// Set default formulas, to have easy mouse move handling at the beginning,
		// if the user decided to use formulas...
		//pTool->SetDefaultFormulas();
...
void SVIPDoc::OnEditDelete() 
{
	...
	if( GetToolSet() && GetToolSetTabView() && ! GetToolSetTabView()->IsLabelEditing() )
	{
		// Delete tool...
		int index;
		if( ( index = GetToolSetTabView()->GetCurSel() ) != -1 )
		{
			// Don't let the first tool be delete if it is a Color Tool and 
			// the next tool isn't a Color Tool
			// A Color Tool must always be first on a RGB Color System.
			SVTaskObjectClass* pTaskObject = GetToolSet()->GetAt( index );
			if( index + 1 < GetToolSet()->GetSize() )
			{
				pNextTool = (SVToolClass*) GetToolSet()->GetAt( index + 1 );
			}// end if

			if( index == 0 )
			{
				if( pTaskObject &&  SV_IS_KIND_OF( pTaskObject, SVColorToolClass ) &&
			...
			if( hr == S_OK )
			{
				...
				int iSize = GetToolSet()->GetSize();
				if( iSize > 0 )
				{
					pTaskObject = GetToolSet()->GetAt( index );
				}
				else
				{
					pTaskObject = NULL;
				...
				SVToolSetTabViewClass* l_pTv = GetToolSetTabView();
				l_pTv->toolSetListCtrl.SaveScrollPos();
				l_pTv->toolSetListCtrl.Rebuild();
				if( pTaskObject )
				{
					m_SelectedToolID = pTaskObject->GetUniqueObjectID();
					l_pTv->toolSetListCtrl.SetCurrentSelection(m_SelectedToolID);
				}

				l_pTv->toolSetListCtrl.SetCurrentSelection( index );
				l_pTv->toolSetListCtrl.RestoreScrollPos(); 
				l_pTv->toolSetListCtrl.EnsureVisible( index, TRUE );
...
// .History
//	 Date		Author		Comment                                       
//  :16.02.1999 RO			First Implementation
////////////////////////////////////////////////////////////////////////////////
void SVIPDoc::OnEditPaste() 
{
	...
	if( GetToolSet() != NULL ) 
	{
		...
		if( hData && ( tStrMem = ( LPTSTR ) GlobalLock( hData ) ) )
		{
			...
			AfxMessageBox( "Parse test..." );
			//if( TheObjectScriptParser.Parse( m_pToolSet, strData ) )
			if ( false )
			{
				// Rebuild the links
				GetToolSet()->SetDefaultInputs();

				AfxMessageBox( "...successfully parse test end!" );
			}
			else
				AfxMessageBox( "...not successfully parse test end!" );
...
/*
	...
	if( GetToolSet() != NULL )
	{
		...
		// Get the Tool Set View
		//SVToolSetViewClass* view = GetToolSetView();
		SVToolSetTabViewClass* view = GetToolSetTabView();

		POSITION pos = view->GetFirstSelectedItemPosition();

		// POSITION pos = GetListCtrl().GetFirstSelectedItemPosition();
		while( pos )
		{
			nItem = view->GetNextSelectedItem( pos );

			//nItem = GetListCtrl().GetNextSelectedItem( pos );
			// Don't access list end marker nor empty list dummy...

			if( nItem != -1 && GetToolSet()->GetToolAt( nItem ) )
				GetToolSet()->GetToolAt( nItem )->GetObjectScript( script, alias );
		}
		alias += script;
...
//	 Date		Author		Comment                                       
//  :dd.mm.yyyy				First Implementation
//	:10.08.1999 RO			Added User Check
////////////////////////////////////////////////////////////////////////////////
...
// .History
//	 Date		Author		Comment                                       
//  :16.06.1999 RO			First Implementation
//	:10.08.1999 RO			Added User Check
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
// .Title       : OnFileSaveImage
// -----------------------------------------------------------------------------
// .Description : Opens Save Image Panel.
////////////////////////////////////////////////////////////////////////////////
// .History
//	 Date		Author		Comment                                       
//  :dd.mm.yyyy				First Implementation
...
// .Title       : AssertValid
// -----------------------------------------------------------------------------
// .Description : Only in Debug version!
//              :
////////////////////////////////////////////////////////////////////////////////
// .History
//	 Date		Author		Comment                                       
...
void SVIPDoc::OnPublishedResultImagesPicker()
{
	...
	if( nResult == IDOK )
	{
		...
		// Force the PPQs to rebuild
		TheSVObserverApp.m_pConfiguration->GetPPQCount( lSize );

		for( l = 0; l < lSize; l++ )
		{
			TheSVObserverApp.m_pConfiguration->GetPPQ( l, &pPPQ );
			if( pPPQ )
			{
				pPPQ->RebuildOutputList();
			}// end if
		...
		TheSVObserverApp.PIODoc->UpdateAllViews( NULL );
		*/
	...
	if( nResult == IDOK )
	{
		...
		for( k = 0; k < l_pInspection->m_PPQInputs.size(); k++ )
		{
			...
			if( l_pInspection->m_PPQInputs[k].m_IOEntryPtr->m_pValueObject->ObjectAttributesSet() & SV_VIEWABLE )
			{
				...
				if( !bFound )
					l_pInspection->m_arViewedInputNames.Add( strName );
			}// end if
			else
			{
				lSize = l_pInspection->m_arViewedInputNames.GetSize();
...
{
	SVResultListClass* pResultList = GetToolSet()->GetResultList();
	if( pResultList )
	{
...
HRESULT SVIPDoc::IsResultDefinitionsUpdated() const
{
	...
	if( pToolSet )
	{
		...
		if( pResultList )
		{
			if( m_PPQListTimestamp < m_ResultDefinitionsTimestamp && pResultList->m_LastUpdateTimestamp < m_ResultDefinitionsTimestamp )
			{
				l_Status = S_FALSE;
			}
		}
...
////////////////////////////////////////////////////////////////////////////////
// .Title       : GetPublishList
// -----------------------------------------------------------------------------
// .Description : ...
////////////////////////////////////////////////////////////////////////////////
// .History
//	 Date		Author		Comment                                       
//  :08.09.1999 SEJ			First Implementation
////////////////////////////////////////////////////////////////////////////////
...
void SVIPDoc::RunRegressionTest()
{
	...
	if ( GetInspectionProcess()->CanRegressionGoOnline() )
	{
		...
		if( TheSVObserverApp.m_svSecurityMgr.SVValidate( SECURITY_POINT_MODE_MENU_REGRESSION_TEST ) == S_OK )
		{
			l_bAllowAccess = true;
		}
...
// .Title       : InitMenu
// -----------------------------------------------------------------------------
// .Description : Loads and inits dyn. Menues.
//              :	Utility Menu
//				:	Tool Set Draw Menu
////////////////////////////////////////////////////////////////////////////////
// .History
//	 Date		Author		Comment                                       
//  :dd.mm.yyyy				First Implementation
//	:22.02.2000 RO			Added Tool Set Draw Menu support.
////////////////////////////////////////////////////////////////////////////////
void SVIPDoc::InitMenu()
{
	...
	// Remove Add Menu if not in Edit Mode 
	if ( !SVSVIMStateClass::CheckState( SV_STATE_EDIT ) )
	{
		TheSVObserverApp.DisplayAddMenu(false);
	}
...
////////////////////////////////////////////////////////////////////////////////
// .Title       : OnUpdateViewToolSetDrawSubMenues
// -----------------------------------------------------------------------------
...
//  :22.02.2000 RO			First Implementation
////////////////////////////////////////////////////////////////////////////////
void SVIPDoc::OnUpdateViewToolSetDrawSubMenues( CCmdUI* PCmdUI ) 
{
...
void SVIPDoc::OnChangeToolSetDrawFlag( UINT nId )
{
	...
	// Access denied...
	// ! TheSVObserverApp.CheckCurrentUserAccess( ID_VIEW_TOOLSETDRAW ) ) old code
	if( ! TheSVObserverApp.OkToEdit() )
		return;
...
void SVIPDoc::RecreateImageSurfaces()
{
	for( int i = 0; i < 8; ++ i )
	{
		...
		if( pImageView )
		{
			pImageView->RecreateImageSurface();
		}
	}
}
...
BOOL SVIPDoc::SetParameters( SVTreeType& rTree, SVTreeType::SVBranchHandle htiParent )
{
	...
	if ( bOk )
	{
			...
			if ( bOk )
			{
				...
				if ( bOk )
				{
					...
					if ( bOk )
					{
						...
						if ( bOk )
						{
							wndpl.ptMinPosition.y = svVariant;
						}
					}
				}
			}
			...
			if ( bOk )
			{
				...
				if ( bOk )
				{
					...
					if ( bOk )
					{
						...
						if ( bOk )
						{
							wndpl.ptMaxPosition.y = svVariant;
						}
					}
				}
			}
			...
			if ( bOk )
			{
				...
				if ( bOk )
				{
					...
					if ( bOk )
					{
						...
						if ( bOk )
						{
							wndpl.rcNormalPosition.top = svVariant;
						}
					}
					...
					if ( bOk )
					{
						...
						if ( bOk )
						{
							wndpl.rcNormalPosition.right = svVariant;
						}
					}
					...
					if ( bOk )
					{
						...
						if ( bOk )
						{
							wndpl.rcNormalPosition.bottom = svVariant;
						}
					}
				}
			}
		...
		if ( bOk )
		{
			...
			//
			//SVToolSetViewClass * pView = ( SVToolSetViewClass * )getView();
			View.pToolSetTabView = (SVToolSetTabViewClass*) getView();
			if( View.pToolSetTabView && View.pToolSetTabView->GetSafeHwnd() )
			{
				CSplitterWnd * pWndSplitter = (CSplitterWnd *) View.pToolSetTabView->GetParent();
...
BOOL SVIPDoc::SetSelectedToolForOperatorMove( BOOL bSetTool )
{
	...
	if( l_Status )
	{
		...
		if( PView && l_Status )
		{
			...
			//
			nIndexTool = PView->toolSetListCtrl.SetCurrentSelection( m_SelectedToolID );
		}
...
void SVIPDoc::OnEditAdjusttoolposition() 
{
	...
	l_pTool = dynamic_cast< SVToolClass* >( SVObjectManagerClass::Instance().GetObject( m_SelectedToolID ) );

	if( l_pTool )
	{
		SVObjectInfoStruct info;
		SVObjectTypeInfoStruct typeInfo;

		info = l_pTool->GetObjectInfo();
		typeInfo = info.ObjectTypeInfo;
...
void SVIPDoc::OnUpdateEditAdjusttoolposition(CCmdUI* pCmdUI) 
{
	...
	BOOL l_bOk = l_pTool != NULL;

	l_bOk = l_bOk && dynamic_cast< SVGageToolClass* >( l_pTool ) == NULL;
...
{
	if( TheSVObserverApp.m_svSecurityMgr.SVValidate( SECURITY_POINT_VIEW_MENU_RESET_COUNTS_ALL ) == S_OK )
	{
...
{
	if( TheSVObserverApp.m_svSecurityMgr.SVValidate( SECURITY_POINT_VIEW_MENU_RESET_COUNTS_CURRENT ) == S_OK )
	{
...
{
	m_regtestRunMode = (RegressionRunModeEnum)newMode;
}
...
CString SVIPDoc::GetCompleteToolSetName() const
{
	...
	if( l_pToolSet != NULL )
	{
		l_Name = l_pToolSet->GetCompleteObjectNameToObjectType( NULL, SVToolObjectType );
	}
...
void SVIPDoc::OnEditDataDefinitionLists() 
{
	...
	strTitle += GetInspectionProcess()->GetName();
...
HRESULT SVIPDoc::UnregisterImage( const SVGUID& p_rImageId, SVImageViewClass* p_pImageView )
{
	...
	if( l_Iter != m_RegisteredImages.end() )
	{
		...
		if( l_Iter->second.empty() )
		{
			m_RegisteredImages.erase( l_Iter );
		}
...
{
	SVInspectionProcess* l_pInspection = GetInspectionProcess();
	BOOL l_Status = ( l_pInspection != NULL );
