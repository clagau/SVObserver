...
HRESULT RangeValidator::IsFieldValid(SVString& rMsg, const SVString& rFieldName, const SVString& rValue)
{
	...
	if (!len)
	{
		rMsg.Format(SvO::RangeValue_EmptyString, rFieldName.c_str());
		hr = E_POINTER;
	}
	else
	{
		...
		if (isNumber)
		{
			...
			{
				rMsg.Format(SvO::RangeValue_WrongRange, rFieldName.c_str(), static_cast<int>(s_RangeMin), static_cast<int>(s_RangeMax));
				hr = E_INVALIDARG;
			}		
		}
	}
