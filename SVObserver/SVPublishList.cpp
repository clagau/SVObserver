...
void SVPublishListClass::Refresh(SVTaskObjectClass * pRootObject)
{
	...
	for( long i = GetSize() - 1; i >= 0; i-- )
	{
		...
		if( !found ) // not found - have to remove it
		{
			...
			// Disconnect
			::SVSendMessage(pPublishedOutObjectInfo->UniqueObjectID, SVM_DISCONNECT_OBJECT_INPUT, ( DWORD )&InObjectInfo, NULL );
			...
			// Get OutputList to remove unpublished items
			pConfig->GetOutputObjectList( &pOutputList );

			SVOutputObjectArray l_Outputs;
	...
	for( int n = 0; n < newList.GetSize(); n++ )
	{
		...
		if( !found) // not in original list - have to add it
		{
			...
			if( ::SVSendMessage( pOutObjectInfo->PObject, SVM_CONNECT_OBJECT_INPUT, (DWORD) &InObjectInfo, NULL ) == SVMR_SUCCESS )
			{
				// *** // ***
				...
				if( pValueObject )
				{
					SVDigitalOutputObject* l_pDigital = NULL;
					CString strName = pValueObject->GetCompleteObjectName();

					pConfig->GetOutputObjectList( &pOutputList );

					pOutputList->GetOutput( strName, l_pDigital );
					...
					if( bDigital )
					{
						...
						{
							pIOEntry->m_IOId = l_pDigital->GetUniqueObjectID();
						}
