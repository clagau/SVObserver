//******************************************************************************
//* COPYRIGHT (c) 20132 by Körber Pharma Inspection GmbH. All Rights Reserved
//* All Rights Reserved
//******************************************************************************
	...
	*pResult = S_OK;
	...
	else
	{
		...
		if (pEdit)
		{
			...
			// Update the control
			std::string Text = std::format("{:d}", m_verticalBinning.value);
	...
	*pResult = S_OK;
	...
	else
	{
		...
		if (pEdit)
		{
			...
			// Update the control
			std::string Text = std::format( _T("{:d}"), m_horizontalBinning.value);
			pEdit->SetWindowText( Text.c_str() );
...
{
	std::string Value = std::format( _T("{:d}"), m_verticalBinning.value );
...
{
	std::string Value = std::format( _T("{:d}"), m_horizontalBinning.value );
...
void SVGigeCameraROIDlg::Normalize( CRect &l_roRect )
{
	...
	Log_Assert( l_roRect.Width() >= 0 );
	Log_Assert( l_roRect.Height() >= 0 );
}
