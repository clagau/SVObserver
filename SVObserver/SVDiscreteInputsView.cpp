...
void SVDiscreteInputsView::OnUpdate( CView* pSender, LPARAM lHint, CObject* pHint )
{
	...
	if( pIODoc && ::IsWindow( m_hWnd ) )
	{
		...
		if ( nullptr == pInputList )
		{
			...
			e.setMessage( SVMSG_SVO_55_DEBUG_BREAK_ERROR, SvOi::Tid_ErrorGettingInputObjectList, StdMessageParams, SvOi::Err_17006_ErrorGettingInputObjectList );
			DebugBreak();
			return; 
		}
		...
		{
			SvStl::MessageMgrNoDisplay e( SvStl::LogOnly );
			e.setMessage( SVMSG_SVO_55_DEBUG_BREAK_ERROR, SvOi::Tid_ErrorFillingInputs, StdMessageParams, SvOi::Err_17007_ErrorFillingInputs );
			DebugBreak();
		}
...
void SVDiscreteInputsView::OnLButtonDblClk( UINT nFlags, CPoint point )
{
		...
		pIODoc )
	{
			...
			( flags & ( LVHT_ONITEMSTATEICON | LVHT_ONITEMICON | LVHT_ONITEMLABEL ) ) )
		{
			...
			{
				SvStl::MessageMgrNoDisplay e( SvStl::LogOnly );
				e.setMessage( SVMSG_SVO_55_DEBUG_BREAK_ERROR, SvOi::Tid_ErrorGettingInputObjectList, StdMessageParams, SvOi::Err_17008_ErrorGettingInputObjectList );
				DebugBreak();
			}
			...
			{
				SvStl::MessageMgrNoDisplay e( SvStl::LogOnly );
				e.setMessage( SVMSG_SVO_55_DEBUG_BREAK_ERROR, SvOi::Tid_ErrorFillingInputs, StdMessageParams, SvOi::Err_17009_ErrorFillingInputs );
				DebugBreak();
			}
