...
//* .Check In Date   : $Date:   23 Apr 2013 14:48:36  $
//******************************************************************************
...
DWORD SVRemoteInputTool::processMessage( DWORD DwMessageID, DWORD DwMessageValue, DWORD DwMessageContext )
{
	//BOOL bResult;
	DWORD l_Status = NULL;
				...
				if( l_Command == _T( "AddDataItems" ) )
				{
					...
					if( l_JsonDataItems.isArray() )
					{
						for (size_t i = 0;i < l_JsonDataItems.size();i++)
						{
							...
							const Json::Value& l_JsonItem = l_JsonDataItems[i];
