...
void SVToolAdjustmentDialogMaskPageClass::OnEditStaticMaskButton() 
{
	if( NULL == m_pMaskEditorCtl )
	{
		...
		if (!bResult)
		{
			...
			Msg.setMessage( SVMSG_SVO_93_GENERAL_WARNING, SvOi::Tid_MaskPage_StartOCXFailed, StdMessageParams, SvOi::Err_10215 );
			return;
		}
	}
...
void SVToolAdjustmentDialogMaskPageClass::OnExportMaskButton() 
{
	...
	if (m_svfnFileName.SaveFile())
	{
		...
		{
			SvStl::MessageMgrDisplayAndNotify Msg( SvStl::LogAndDisplay );
			Msg.setMessage( SVMSG_SVO_93_GENERAL_WARNING, SvOi::Tid_Error_CannotOpenFile, StdMessageParams, SvOi::Err_10216 );
		}
	}
	// Refresh image view...
	initMask();
...
void SVToolAdjustmentDialogMaskPageClass::OnImportMaskButton() 
{
	...
	if (m_svfnFileName.SelectFile())
	{
		...
		{
			SvStl::MessageMgrDisplayAndNotify Msg( SvStl::LogAndDisplay );
			Msg.setMessage( SVMSG_SVO_93_GENERAL_WARNING, SvOi::Tid_Error_CannotOpenFile, StdMessageParams, SvOi::Err_10217 );
		}
	}
	// Refresh image view...
	initMask();
