...
//* .Check In Date   : $Date:   23 Apr 2013 15:31:38  $
//******************************************************************************
...
SVToolAdjustmentDialogFilterPageClass::SVToolAdjustmentDialogFilterPageClass( SVToolAdjustmentDialogSheetClass* Parent ) 
	...
	if( pParentDialog )
	{
		...
		if (pTool)
		{
			...
			pUnaryImageOperatorList = 
				( SVUnaryImageOperatorListClass* )SVSendMessage( pTool, SVM_GETFIRST_OBJECT, NULL, ( DWORD )&info );

			if( pUnaryImageOperatorList )
			{
				info.ObjectType = SVFilterObjectType;
				SVSendMessage( pUnaryImageOperatorList, SVM_GETAVAILABLE_OBJECTS, ( DWORD )&availableFilters, ( DWORD )&info );
			}
		}
	}
}
	...
	if( index != LB_ERR ) 
	{
		...
		{ 
			...
			m_btnProperties.EnableWindow( SV_IS_KIND_OF( pOperator, SVCustomFilterClass ) 
				...
				|| SV_IS_KIND_OF( pOperator, SVWatershedFilterClass ) );
		}// end if
	...
	if( sourceIndex != LB_ERR )
	{
		...
		if(  sourceIndex != -1 )
		{
			...
			if (pUnaryImageOperatorList )
			{
				...
				SVFilterClass* pFilter = NULL;

				SVObjectManagerClass::Instance().ConstructObject( availableFilters.GetAt( sourceIndex ).ClassId, pFilter );
				...
				if( pFilter )
				{
					...
					if( ! pFilter->IsCreated() )
					{
						...
						if( ::SVSendMessage( pUnaryImageOperatorList, SVM_CREATE_CHILD_OBJECT, ( DWORD ) pFilter, SVMFResetObject ) != SVMR_SUCCESS )
						{
							...
							if( objectID != SVInvalidGUID )
								pUnaryImageOperatorList->Delete( objectID );
							else
								delete pFilter;
						}
					}
...
void SVToolAdjustmentDialogFilterPageClass::OnButtonClearAll() 
{
	...
	if( pUnaryImageOperatorList )
	{
		...
		for( int i = pUnaryImageOperatorList->GetSize()-1; i >= 0; --i )
		{
			SVFilterClass *l_pFilter = dynamic_cast<SVFilterClass *>( pUnaryImageOperatorList->GetAt( i ) );
			...
			if( l_pFilter != NULL )
			{
				...
				::SVSendMessage( pUnaryImageOperatorList, SVM_DESTROY_CHILD_OBJECT, ( DWORD )l_pFilter, NULL );
			}
		}
		...
		if( l_bReset )
		{
			...
			pTool->GetInspection()->SetDefaultInputs();

			::SVSendMessage( pTool->GetInspection(), SVM_RESET_ALL_OBJECTS, NULL, NULL );
		}
...
void SVToolAdjustmentDialogFilterPageClass::OnButtonDeleteCurrentFilter() 
{
	...
	if( index != LB_ERR ) 
	{
		if( pUnaryImageOperatorList )
		{ 
			SVFilterClass *l_pFilter = dynamic_cast<SVFilterClass *>( (SVObjectClass *)filterListBox.GetItemData( index ) );
			...
			if( l_pFilter != NULL )
			{
				...
				if( l_pFilter->ShouldResetIPDoc() )
				{
					l_Context = SVMFSetDefaultInputs | SVMFResetInspection;
				}

				// Close, Disconnect and Delete it
				::SVSendMessage( pUnaryImageOperatorList, SVM_DESTROY_CHILD_OBJECT, ( DWORD )l_pFilter, l_Context );
			}
...
void SVToolAdjustmentDialogFilterPageClass::OnButtonProperties() 
{
	...
	if( index != LB_ERR ) 
	{
		...
		{ 
			...
			SVWatershedFilterClass *l_pWatershedFilter = dynamic_cast<SVWatershedFilterClass *>( l_pOperator );
			...
			{
				SVRankingFilterDlg l_svDlg;

				l_svDlg.SetTaskObject( l_pRankingFilter );
			...
			{
				SVThickeningFilterDlg l_svDlg;

				l_svDlg.SetTaskObject( l_pThickeningFilter );

				l_svDlg.DoModal();
			}// end if
			else if( l_pWatershedFilter != NULL )
			{
				SVWatershedFilterDlg l_svDlg;
				l_svDlg.m_pParent = pParentDialog;

				l_svDlg.SetTaskObject( l_pWatershedFilter );
