...
//* All Rights Reserved
//******************************************************************************
//* .Module Name     : TADlgMaskPageClass
//* .File Name       : $Workfile:   TADlgMaskPageClass.cpp  $
//* ----------------------------------------------------------------------------
...
TADlgMaskPageClass* TADlgMaskPageClass::m_pThis = nullptr;

BEGIN_MESSAGE_MAP(TADlgMaskPageClass, CPropertyPage)
	//{{AFX_MSG_MAP(TADlgMaskPageClass)
...
TADlgMaskPageClass::TADlgMaskPageClass(uint32_t inspectionId, uint32_t taskObjectId, uint32_t maskOperatorId)
	: CPropertyPage(TADlgMaskPageClass::IDD)
	...
	, m_maskController{ inspectionId, taskObjectId, maskOperatorId }
{
	...
	//{{AFX_DATA_INIT(TADlgMaskPageClass)
...
TADlgMaskPageClass::~TADlgMaskPageClass()
{
	m_pThis = nullptr;
	if (m_pMaskEditorCtl)
	{
...
void TADlgMaskPageClass::SetupMaskOperatorComboBox()
{
...
void TADlgMaskPageClass::SetupFillAreaComboBox()
{
...
void TADlgMaskPageClass::SetupDrawCriteriaCombo()
{
...
void TADlgMaskPageClass::SetInspectionData()
{
	UpdateData(true);
	...
	{
		long Value = static_cast<long> (m_cbMaskOperator.GetItemData(CurrentSelection));
	...
	if (CB_ERR != CurrentSelection)
	{
		long Value = static_cast<long> (m_cbFillOptions.GetItemData(CurrentSelection));
	...
	if (CB_ERR != CurrentSelection)
	{
		long Value = static_cast<long> (m_DrawCriteriaCombo.GetItemData(CurrentSelection));
...
void TADlgMaskPageClass::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(TADlgMaskPageClass)
...
BOOL TADlgMaskPageClass::OnSetActive()
{
	setImages();
...
// .Title       : initMask
////////////////////////////////////////////////////////////////////////////////
void TADlgMaskPageClass::initMask()
{
	SetInspectionData();
	setImages();
...
BOOL TADlgMaskPageClass::OnInitDialog()
{
...
void TADlgMaskPageClass::OnEditStaticMaskButton()
{
...
void TADlgMaskPageClass::OnExportMaskButton()
{
	UpdateData(true);
...
void TADlgMaskPageClass::OnImportMaskButton()
{
	UpdateData(true);
...
void TADlgMaskPageClass::OnEditShapeMaskButton()
{
	m_Values.Commit(SvOgu::PostAction::doReset | SvOgu::PostAction::doRunOnce);
...
void TADlgMaskPageClass::OnActivateMask()
{
	initMask();
...
void TADlgMaskPageClass::OnSelchangeMaskOperatorCombo()
{
	initMask();
}

void TADlgMaskPageClass::OnRadioImageMask()
{
	UpdateData();
	ShowControls(m_iMaskType);
	initMask();
...
void TADlgMaskPageClass::OnRadioStaticMask()
{
	UpdateData();
	ShowControls(m_iMaskType);
	initMask();
...
void TADlgMaskPageClass::OnRadioShapeMask()
{
	UpdateData();
	ShowControls(m_iMaskType);
	initMask();
...
void TADlgMaskPageClass::ShowControls(int p_iMaskType)
{
	// Fill
...
void TADlgMaskPageClass::SetupImageMaskCombo()
{
...
void TADlgMaskPageClass::OnSelchangeComboImageMask()
{
...
void TADlgMaskPageClass::OnButtonFillColorMore()
{
	UpdateData();
	CColorDialog dlg;
...
void TADlgMaskPageClass::OnChangeEditFillColor()
{
	initMask();
}

UINT_PTR CALLBACK TADlgMaskPageClass::ColorDlgHookFn(HWND hdlg, UINT uiMsg, WPARAM, LPARAM lParam)
{
...
void TADlgMaskPageClass::OnSelchangeComboFillOptions()
{
	initMask();
}

void TADlgMaskPageClass::setImages()
{
	IPictureDisp* pSourceImage = m_maskController.GetReferenceImage();
...
void TADlgMaskPageClass::OnSelchangeDrawMaskCriteria()
{
	initMask();
}
