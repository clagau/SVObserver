...
STDMETHODIMP CSVCommand::GetSVIMState(BSTR *pszXMLData, BSTR *pXMLError)
{
	...
	do
	{
		...
		*pszXMLData = SysAllocString(XMLData);
		...
		Exception.setMessage( SVMSG_CMDCOMSRV_NO_ERROR, SvOi::Tid_Empty, SvStl::SourceFileParams(StdMessageParams) );
		break;
xmlerror:
		//create an exception object for an XML error
		if(!SvXmlCmd.GetParserErrorCode())
		{
			Exception.setMessage( SVMSG_CMDCOMSRV_XML_ERROR, SvOi::Tid_Empty, SvStl::SourceFileParams(StdMessageParams) );
		}
		else
		{
...
STDMETHODIMP CSVCommand::PutSVIMConfig(BSTR szXMLData, BSTR* pXMLError)
{
	...
	do
	{
		Exception.setMessage( SVMSG_CMDCOMSRV_NO_ERROR, SvOi::Tid_Command_StartSvimConfig, SvStl::SourceFileParams(StdMessageParams) );
		...
		Exception.setMessage( SVMSG_CMDCOMSRV_NO_ERROR, SvOi::Tid_Command_EndSvimConfig, SvStl::SourceFileParams(StdMessageParams) );
		break;
error:
		//set the exception for the correct reason for breaking
		Exception.setMessage( hrException, SvOi::Tid_Empty, SvStl::SourceFileParams(StdMessageParams) );
		hrResult = S_FALSE;
		break;
xmlerror:
		//create an exception object for an XML error
...
STDMETHODIMP CSVCommand::LoadSVIMConfig(BSTR szXMLData, BSTR* pXMLError)
{
	...
	SVXmlException svXmlException;
	SVXmlCmd SvXmlCmd;
	LPTSTR szConfigFileName = nullptr;
	TCHAR szDrive[_MAX_DRIVE], szDir[_MAX_DIR], szFName[_MAX_FNAME], szExt[_MAX_EXT];
	...
	do
	{
		...
		if( ! SVSVIMStateClass::CheckState( SV_STATE_LOADING ) )
		{
			...
			if( pIPDoc )
			{
				...
				if( S_OK != pIPDoc->GetResults( pSVRXW ) )
				{
					goto error;
				}
			}
...
STDMETHODIMP CSVCommand::SVRegisterStream(SAFEARRAY* psaName, VARIANT vtInterface, SAFEARRAY** ppsaStatus)
{
	...
	try 
	{
		...
		{ // registration already happened...
			...
			if( S_OK != hrRet )
			{
				Exception.setMessage( hrRet, SvOi::Tid_Empty, SvStl::SourceFileParams(StdMessageParams) );
			}
	...
	if (TheSVOLicenseManager().HasMatroxIdentificationLicense())
	{
			...
			TheSVObserverApp.m_mgrRemoteFonts.UpdateFontTime( lFontIdentifier ))
		{
			...
			if( nullptr != bstrFontFile )
			{
				...
				SVString l_strFontFileName = strFontFileName;
				...
				if ( l_Code != SVMEE_STATUS_OK )
				{
					...
					Exception.setMessage( hr, SvOi::Tid_ErrorMIL, msgList, SvStl::SourceFileParams(StdMessageParams) );
				}
			}// end if
			...
			if( nullptr != bstrFontControls )
			{
				...
				SVString l_strControlsFileName = strControlsFileName;
			...
			if( nullptr != bstrFontConstraints )
			{
				...
				oFile.Close();
				SVString l_strConstraintsFileName = strConstraintsFileName;
...
namespace local
{
	HRESULT SafeArrayToScalarValueVector( SAFEARRAY* psaNames, SAFEARRAY* psaValues, SVScalarValueVector& rvec )
	{
		HRESULT hr = S_FALSE;
		long lNumberOfElements = psaNames->rgsabound[0].cElements;
		ASSERT( nullptr == psaValues || lNumberOfElements == psaValues->rgsabound[0].cElements );
		if ( nullptr == psaValues || lNumberOfElements == psaValues->rgsabound[0].cElements )
		{
			for ( long l = 0; l < lNumberOfElements; ++l )
			{
				BSTR bstrName = nullptr;
...
STDMETHODIMP CSVCommand::SVSetConditionalHistoryList(BSTR bstrInspectionName, SAFEARRAY*  psaValueNames,  SAFEARRAY*  psaImageNames,  SAFEARRAY*  psaConditionalNames,  SAFEARRAY** ppsaValueStatus, SAFEARRAY** ppsaImageStatus, SAFEARRAY** ppsaConditionalStatus)
{
	...
	if ( SVConfigurationObject::GetInspection( W2T(bstrInspectionName), pInspection ) )
	{
		...
		// These params are optional, so pass nullptr in SetConditionalHistoryList if not supplied
		SVScalarValueVector* pvecValues = nullptr;
		SVScalarValueVector* pvecImages = nullptr;
		SVScalarValueVector* pvecConditionals = nullptr;

		if ( psaValueNames )
		{
			pvecValues = &vecValues;
			hr = local::SafeArrayToScalarValueVector( psaValueNames, nullptr, vecValues );
		}

		if ( psaImageNames )
		{
			pvecImages = &vecImages;
			hr = local::SafeArrayToScalarValueVector( psaImageNames, nullptr, vecImages );
		}

		if ( psaConditionalNames )
		{
			pvecConditionals = &vecConditionals;
			hr = local::SafeArrayToScalarValueVector( psaConditionalNames, nullptr, vecConditionals );
		}
...
STDMETHODIMP CSVCommand::SVGetConditionalHistoryList( BSTR bstrInspectionName, SAFEARRAY** ppsaValueNames, SAFEARRAY** ppsaImageNames, SAFEARRAY** ppsaConditionalNames, SAFEARRAY** ppsaValueStatus, SAFEARRAY** ppsaImageStatus, SAFEARRAY** ppsaConditionalStatus )
{
	...
	if ( SVConfigurationObject::GetInspection( W2T(bstrInspectionName), pInspection ) )
	{
		...
		if ( S_OK == hr )
		{
			HRESULT hrTemp = local::ScalarValueVectorToSafeArray( vecValues, ppsaValueNames, nullptr, ppsaValueStatus );
			hrTemp = local::ScalarValueVectorToSafeArray( vecImages, ppsaImageNames, nullptr, ppsaImageStatus );
			hrTemp = local::ScalarValueVectorToSafeArray( vecConditionals, ppsaConditionalNames, nullptr, ppsaConditionalStatus );
		}
	}
	else
	{
