...
{ 
	...
	if (nullptr != pbValue)
	{
		if (ulChannel < g_Plc.GetInputCount())
		{
			bool bState;
			result = g_Plc.GetInputBit(ulChannel, bState);
...
HRESULT WINAPI SVOutputSetValue(unsigned long channel, bool bValue)
{
	...
	if (channel < g_Plc.GetOutputCount())
	{
		result = g_Plc.SetOutputBit(channel, bValue);
	}
	return result;
}
...
HRESULT WINAPI SVOutputSetPortValue(unsigned long port, unsigned long value)
{
	...
	if (port < g_Plc.GetPortCount())
	{
		result = g_Plc.SetOutputValue(value);
	}
	return result;
}
...
HRESULT WINAPI SVOutputSetData(unsigned long triggerchannel, const SvTh::IntVariantMap& rData)
{
	...
	if (0 < triggerchannel && cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.SetOutputData(triggerchannel, rData);
	}
	return result;
}
...
HRESULT WINAPI SVTriggerGetHandle(unsigned long* pTriggerchannel, unsigned long index)
{
	...
	{
		*pTriggerchannel = g_Plc.GetTriggerHandle(index);
		result = S_OK;
	}
	return result;
}
...
HRESULT WINAPI SVTriggerGetName(unsigned long triggerchannel, BSTR* pName)
{
	...
	if (nullptr != pName && 0 < triggerchannel && cNumberTriggers >= triggerchannel)
	{
		...
		*pName = g_Plc.GetTriggerName(triggerchannel);
...
HRESULT WINAPI SVTriggerRegister(unsigned long triggerchannel, const SvTh::TriggerDispatcher &rDispatcher)
{
	...
	if ( rDispatcher.hasCallback() && 0 < triggerchannel && cNumberTriggers >= triggerchannel)
	{
		result = S_OK;

		g_Plc.AddDispatcher(triggerchannel, rDispatcher);
	} 
	return result;
}
...
HRESULT WINAPI SVTriggerUnregister(unsigned long triggerchannel, const SvTh::TriggerDispatcher &rDispatcher)
{
	...
	if (rDispatcher.hasCallback() && 0 < triggerchannel && cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.RemoveDispatcher(triggerchannel, rDispatcher);
	} 
	return result;
}
...
HRESULT WINAPI SVTriggerUnregisterAll(unsigned long triggerchannel)
{
	...
	if (0 < triggerchannel && cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.RemoveAllDispatchers(triggerchannel);
	} 
	return result;
}
...
HRESULT WINAPI SVTriggerStart(unsigned long triggerchannel)
{
	...
	if (0 < triggerchannel && cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.StartTrigger(triggerchannel);
	}
	return result;
}
...
HRESULT WINAPI SVTriggerStop(unsigned long triggerchannel)
{
	...
	if (0 < triggerchannel && cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.StopTrigger(triggerchannel);
	} 
	return result;
}
...
HRESULT WINAPI SVTriggerGetParameterCount(unsigned long triggerchannel, unsigned long *pCount)
{
	...
	if (0 < triggerchannel && cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.TriggerGetParameterCount(triggerchannel, pCount);
	}
	return result;
}
...
HRESULT WINAPI SVTriggerGetParameterName(unsigned long triggerchannel, unsigned long index, BSTR* pName)
{
	...
	if (cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.TriggerGetParameterName(triggerchannel, index, pName);
	}
	return result;
}
...
HRESULT WINAPI SVTriggerGetParameterValue(unsigned long triggerchannel, unsigned long ulIndex, VARIANT* pValue)
{
	...
	if (cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.TriggerGetParameterValue(triggerchannel, ulIndex, pValue);
	}
	return result;
}
...
HRESULT WINAPI SVTriggerSetParameterValue(unsigned long triggerchannel, unsigned long ulIndex, VARIANT* pValue)
{
	...
	if (cNumberTriggers >= triggerchannel)
	{
		result = g_Plc.TriggerSetParameterValue(triggerchannel, ulIndex, pValue);
	}
	return result;
}
...
{
	return g_Plc.GetParameterCount(pCount);
}
...
{
	return g_Plc.GetParameterName(index, pName);
}
...
{
	return g_Plc.GetParameterValue(index, pValue);
}
...
{
	return g_Plc.SetParameterValue(index, pValue);
}
