...
//-			considered an error.
		...
		if( bAutoThreshold )
		{
			...
			if( /*m_pHistValueArray &&*/ m_histValueArraySize && m_pixelNumber )
			{
				...
				if( bUseExternalATM )
				{
					SVDoubleValueObjectClass* pExtATMValue = SvOl::getInput<SVDoubleValueObjectClass>(m_inputATM, true);
		...
		else
		{
			...
			if( bUpperThreshActive )
			{
				...
				{
					double dUpper = 0.0;
					SVDoubleValueObjectClass* pExtUTValue = SvOl::getInput<SVDoubleValueObjectClass>(m_inputUT, true);
			...
			if( bLowerThreshActive )
			{
				...
				{
					double dLower = 0.0;
					SVDoubleValueObjectClass* pExtLTValue = SvOl::getInput<SVDoubleValueObjectClass>(m_inputLT, true);
	...
	if( Result )
	{
		...
		if( M_NULL != m_histResultID )
		{
			...
			if ( pTool && S_OK == pTool->GetImageExtent( l_svExtents ) &&
				S_OK == l_svExtents.GetExtentProperty( SvDef::SVExtentPropertyOutputWidth, l_dWidth ) &&
				S_OK == l_svExtents.GetExtentProperty( SvDef::SVExtentPropertyOutputHeight, l_dHeight ) )
			{
