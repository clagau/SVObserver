...
namespace SvTi
{
	...
	{
		bool bRetVal = false;
		SvTh::SVTriggerHandlerList::const_iterator it = m_triggerHandlerList.find(p_ulIndex);
		if (it != m_triggerHandlerList.end())
			bRetVal = true;
		return bRetVal;
	}

	HRESULT SVInternalTrigger::Subscribe(unsigned long p_ulIndex, SvTh::SVTriggerHandler& handler)
	{
		HRESULT hr = S_OK;
	...
	{
		HRESULT hr = S_FALSE;
		SvTh::SVTriggerHandlerList::iterator it = m_triggerHandlerList.find(p_ulIndex);
		if (it != m_triggerHandlerList.end())
		{
	...
	{
		SvTh::SVTriggerHandlerList::iterator it;
