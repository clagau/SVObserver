...
VOID CALLBACK SVODataDeviceClass::APCProc( ULONG_PTR  )
{
}
...
SVODataDeviceClass::SVODataDeviceClass(LPCTSTR Name)
: m_DeviceName(Name)
, mbIsCreated(false)
, mbIsValid(false)
, mbIsStarted(false)
...
*/
HRESULT SVODataDeviceClass::Create(unsigned long )
{
	HRESULT hrOk = S_OK;
...
HRESULT SVODataDeviceClass::Start()
{
	...
	if ( mbIsValid )
	{
		hrOk = m_Thread.Create( &SVODataDeviceClass::APCProc, SVProcessFunctorImpl( this ), m_DeviceName.c_str(), SVAffinityAcq  );
...
*/
HRESULT SVODataDeviceClass::Process( bool&  )
{
	HRESULT hrOk = S_OK;
...
*/
HRESULT SVODataDeviceClass::ExecuteCallback( SVCallbackClassPtrQueue&, SVODataResponseClass& p_rResponse )
{
	HRESULT hrOk = S_FALSE;
