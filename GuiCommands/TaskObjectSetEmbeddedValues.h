	...
	namespace GuiCommand
	{
		...
		struct TaskObjectSetEmbeddedValues : public boost::noncopyable
		{
			...
			HRESULT Execute()
			{
				HRESULT hr = S_OK;
				//fill parentMap
				std::map<GUID, SvOi::SetValueObjectPairVector> parentMap;
				...
				for( std::map<GUID, SvOi::SetValueObjectPairVector>::iterator it = parentMap.begin(); it != parentMap.end() && S_OK == hr; ++it )
				{
