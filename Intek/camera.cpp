//******************************************************************************
//* .Module Name     : CCamera
//* .File Name       : $Workfile:   camera.cpp  $
//* ----------------------------------------------------------------------------
//* .Current Version : $Revision:   1.0  $
//* .Check In Date   : $Date:   29 Apr 2013 09:03:04  $
//******************************************************************************
...
// SVR - deviation from Standard Intek FireBus API
char* TriggerModeStr[] =
{
  "intern","extern F", "extern R", "mode15", "oneshot", "software"
};
...
CCamera::CCamera()
{
  ...
  // SVR - deviation from Standard Intek FireBus API
  // SEJ (Begin) - added the initializations below
  m_FrmInfoReadIx = 0;
  m_FrmInfoWriteIx = 0;
  m_FrmDisabled = true;          // Frame queue is disabled (for shutdown)

  m_UserFrameCnt = 0;

  m_TotalFrmCnt = 0;          // Total frames seen
  m_DiscardedFrmCnt = 0;      // Discarded frames
...
////////////////////////////////////////////////////////////////////////////////
// SVR - Do Software trigger
UINT8 CCamera::DoSoftwareTrigger(UINT32 ulRegister, UINT32 ulValue)
{
	return WriteQuad(HIGHOFFSET, m_Props.CmdRegBase + ulRegister, ulValue);
}
...
{
	// SVR - deviation from Standard Intek FireBus API
	UINT8 Result = WriteQuad(HIGHOFFSET,m_Props.CmdRegBase+CCR_ISOENABLE,0);
	
	// SVR - reset to default
	m_Parms.TriggerMode = TM_INTERN;
	return Result;
	// SVR - End deviation from Standard Intek FireBus API
}
...
UINT8 CCamera::StartCamera()
{
  ...
  {
    Mode=0;
	if(m_Parms.TriggerMode==TM_ONESHOT)
     Mode=0x80000000;
    ...
    if(m_Parms.TriggerMode==TM_EXTERN)
     Mode=0x82000000;
		...
		// SVR - deviation from Standard Intek FireBus API
    if(m_Parms.TriggerMode==TM_EXTERN_RISING)
     Mode=0x83000000;
    
		if(m_Parms.TriggerMode==TM_MODE15)
			 Mode=0x820F0000;
	
    if(m_Parms.TriggerMode==TM_SOFTWARE)
     Mode=0x82E00000;
    
		if( Mode != 0 ) // SVR - end deviation from Standard Intek FireBus API
		{
...
UINT8 CCamera::GetFrame(FCTLFRMINFO *pInfo,UINT32 TimeoutInMs,UINT32 *pNumber)
{
  ...
  {
	// SVR - deviation from Standard Intek FireBus API
	TRACE("CCamera::GetFrame - Timeout %d\n", TimeoutInMs);
	// SVR - end deviation from Standard Intek FireBus API
	return FALSE;
  }
  ...
  if(m_pFrmInfo!=NULL)
  {
		...
		{
			// SVR - deviation from Standard Intek FireBus API
			TRACE0("CCamera::GetFrame - m_FrmInfoReadIx == m_FrmInfoWriteIx\n");
			// SVR - end deviation from Standard Intek FireBus API
		}
  }
...
//******************************************************************************
/*
$Log:   N:\PVCSarch65\ProjectFiles\archives\SVObserver_src\Intek\camera.cpp_v  $
 * 
 *    Rev 1.0   29 Apr 2013 09:03:04   bWalter
 * Project:  SVObserver
 ...
 * /////////////////////////////////////////////////////////////////////////////////////
 * 
 *    Rev 1.4   14 Oct 2008 12:13:12   sjones
 * Project:  SVObserver
 * Change Request (SCR) nbr:  612
 * SCR Title:  Implement a timer trigger object
 * Checked in by:  sJones;  Steve Jones
 * Change Description:  
 *   Revised StopIso and StopCamera methods to reset TriggerMode to the default.
 * 
 * /////////////////////////////////////////////////////////////////////////////////////
 * 
 *    Rev 1.3   10 Sep 2008 10:58:30   sjones
 * Project:  SVObserver
 * Change Request (SCR) nbr:  612
 * SCR Title:  Implement a timer trigger object
 * Checked in by:  sJones;  Steve Jones
 * Change Description:  
 *   Added check for OneShot Mode
 * 
 * /////////////////////////////////////////////////////////////////////////////////////
 * 
 *    Rev 1.2   28 Nov 2006 17:24:10   sjones
 * Project:  SVObserver
 * Change Request (SCR) nbr:  582
 * SCR Title:  Upgrade Intek FireStack Driver and API to version 2.15
 * Checked in by:  sJones;  Steve Jones
 * Change Description:  
 *   Added comments around SVResearch revisions
 * Revised per changes in Intek FirePackage v 2.15
 * 
 * /////////////////////////////////////////////////////////////////////////////////////
 * 
 *    Rev 1.1   26 Jul 2006 14:59:38   Joe
 * Project:  SVObserver
 * Change Request (SCR) nbr:  567
 * SCR Title:  Update SVObserver to accept Intek driver for D-series SVIMs
 * Checked in by:  eBeyeler;  Eric Beyeler
 * Change Description:  
 *   Update function to check if the image is sizable.  If so, check if a partial scan format will work.  Otherwise, try to find some standard format.  If the image is not scableable, then check for a standard format, and then check for a partial scan format.
 * 
 * /////////////////////////////////////////////////////////////////////////////////////
 * 
 *    Rev 1.0   14 Dec 2005 09:39:22   Joe
 * Project:  SVObserver
