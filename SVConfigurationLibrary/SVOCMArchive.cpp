...
HRESULT SVOCMArchive::GetSVOSavedVersionNumberFromConfigurationFile(BSTR abstrFileName, unsigned long* apulSVOConfigVersion)
{
	...
	while (1)
	{
		...
		if (FALSE == fConfigurationFile.Open (bstrConfigurationFileName, CFile::modeRead|CFile::shareExclusive, &fileException))
		{
		...
		lNbrOfCharactersRead = fConfigurationFile.Read (csBuffer, sizeof(csBuffer) - 1);
		...
		hr = FindVersion440_ (csBuffer, &ulConfigurationVersion);
		if (hr & 0xc0000000)
		{
			break;
		}
		else if (ulConfigurationVersion != 0)
		{
			//-		Version number found.  No need to continue search.
			break;
		}

		hr = FindVersion401_430 (csBuffer, &ulConfigurationVersion);
...
// @TODO - Refactor this mess
HRESULT SVOCMArchive::FindVersion401_430 (char* apczSourceString, unsigned long* aplConfigurationVersion)
{
	char* pntr1(nullptr);
	char* pntr2(nullptr);
	...
	*aplConfigurationVersion = 0;
	...
	while (1)
	{
		...
		if (nullptr == (pntr1 = strstr (pntr1, "name=\"VersionNumber\"")))
		{
...
//- aplConfigurationVersion to zero.
// @TODO - Refactor this mess
HRESULT SVOCMArchive::FindVersion440_ (char* apczSourceString, unsigned long* p_ulpConfigurationVersion)
{
	HRESULT hr = S_OK;
	...
	while (1)
	{
		hr = FindVersion440__Standard (apczSourceString, &l_ulVersionNumber);
		if (hr & 0xc0000000)
		{
			break;
		}

		if (0 != l_ulVersionNumber)
		{
			break;
		}
		...
		hr = FindVersion440__RevisionHistory (apczSourceString, &l_ulVersionNumber);
...
// @TODO - Refactor this mess
HRESULT SVOCMArchive::FindVersion440__Standard (char* apczSourceString, unsigned long* aplConfigurationVersion)
{
	char* pntr1(nullptr);
	char* pntr2(nullptr);
	char czWhiteSpace [40];
	...
	*aplConfigurationVersion = 0;
	...
	while (1)
	{
		...
		bInQuotes = false;
		while ((('<' != *pntr2) || bInQuotes) && (pntr2 > apczSourceString))
		{
...
// @TODO - Refactor this mess
HRESULT SVOCMArchive::FindVersion440__RevisionHistory (char* p_czpSourceString, unsigned long*	p_ulpConfigurationVersion)
{
	char* pntr1(nullptr);
	char* pntr2(nullptr);
	char czWhiteSpace [40];
	...
	*p_ulpConfigurationVersion = 0;
	...
	while (1)
	{
		...
		if (nullptr == (pntr1 = strstr (pntr1, "FormatVersion=\"")))
		{
			//-		The version number was not found.
			//-		This will be given a default version number.
			break;
