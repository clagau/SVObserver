...
namespace Seidenader
{
	...
	SVCustomFilterDlg::SVCustomFilterDlg(SvOi::ICustomFilter& rFilterObject, CWnd* pParent): 
		CDialog(SVCustomFilterDlg::IDD, pParent)
		,m_pCustomFilterClass(rFilterObject)
	{
	...
	HRESULT SVCustomFilterDlg::SetInspectionData()
	{
		...
		if( l_hrOk == S_OK )
		{
			l_hrOk = m_pCustomFilterClass.addKernelWidthRequest( m_lKernelWidth );
		}

		if( l_hrOk == S_OK )
		{
			l_hrOk = m_pCustomFilterClass.addKernelHeightRequest( m_lKernelHeight );
		}

		if( l_hrOk == S_OK )
		{
			l_hrOk = m_pCustomFilterClass.addClippingOnRequest( TRUE == m_bClippingOn );
		}

		if( l_hrOk == S_OK )
		{
			l_hrOk = m_pCustomFilterClass.addAbsoluteValueRequest( TRUE == m_bAbsoluteValue );
		}

		if( l_hrOk == S_OK )
		{
			l_hrOk = m_pCustomFilterClass.addTransformationFactorRequest( m_lTransformationFactor );
		}
		...
		for( long j = 0; l_hrOk == S_OK && j < m_lKernelWidth; j++ )
		{
			for( long k = 0; l_hrOk == S_OK && k < m_lKernelHeight; k++ )
			{
				...
				l_hrOk = m_pCustomFilterClass.addKernelCellRequest(l_lKernelIndex, m_lKernelCells[ l_lDialogIndex ] );
			}// end for
		...
		if( l_hrOk == S_OK )
		{
			l_hrOk = m_pCustomFilterClass.AddInputRequestMarker();
		}

		if( l_hrOk == S_OK )
		{
			l_hrOk = m_pCustomFilterClass.RunOnce( m_pCustomFilterClass.GetToolInterface() );
		}
	...
	BOOL SVCustomFilterDlg::OnInitDialog()
	{
		...
		m_lKernelWidth = m_pCustomFilterClass.getKernelWidth();
		strTemp.Format( "%d", m_lKernelWidth );
		m_ctlKernelWidth.SelectString( -1, strTemp );

		m_lKernelHeight = m_pCustomFilterClass.getKernelHeight();
		strTemp.Format( "%d", m_lKernelHeight );
		m_ctlKernelHeight.SelectString( -1, strTemp );

		EnableCells();

		m_bClippingOn = m_pCustomFilterClass.isClippingOn();
		m_bAbsoluteValue = m_pCustomFilterClass.isAbsoluteValue();
		m_lTransformationFactor = m_pCustomFilterClass.getTransformationFactor();
		...
		for( long h = 0; h < m_lKernelWidth; h++ )
		{
			for( long i = 0; i < m_lKernelHeight; i++ )
			{
				...
				m_lKernelCells[ l_lDialogIndex ] = m_pCustomFilterClass.getKernelCellValue(l_lKernelIndex);
			}// end for
		}// end for
