	...
	if (GUID_NULL != filterGUID)
	{
		...
		if (S_OK == hr)
		{
			...
			m_btnProperties.EnableWindow(SvDef::SVCustomFilterObjectType == info.SubType
				...
				|| SvDef::EraseBorderBlobsFilterObjectType == info.SubType
				|| SvDef::ExtractHolesFilterObjectType == info.SubType
				|| SvDef::FillHolesFilterObjectType == info.SubType
				|| SvDef::ReconstructFilterObjectType == info.SubType);
		}
	}// end if
