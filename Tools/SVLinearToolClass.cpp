...
bool SVLinearToolClass::CreateObject( const SVObjectLevelCreateStruct& rCreateStructure )
{
	...
	UINT Attributes = SvPb::remotelySetable | SvPb::audittrail;
...
bool SVLinearToolClass::ResetObject(SvStl::MessageContainerVector *pErrorMessages)
{
	...
	if ( S_OK != hrOk )
	{
		...
		{
			SvStl::MessageContainer Msg( SVMSG_SVO_92_GENERAL_ERROR, SvStl::Tid_UpdateLinearToolDataFailed, SvStl::SourceFileParams(StdMessageParams), getObjectId() );
	...
	if ( Result )
	{
		...
		else
		{
			...
			if (nullptr != pErrorMessages)
			{
				SvStl::MessageContainer Msg( SVMSG_SVO_92_GENERAL_ERROR, SvStl::Tid_GetImageExtentFailed, SvStl::SourceFileParams(StdMessageParams), getObjectId() );
				pErrorMessages->push_back(Msg);
			}
		}
	}
...
void SVLinearToolClass::init()
{
	...
	RegisterEmbeddedObject( &m_svRotationAngle, SvPb::RotationAngleEId, IDS_OBJECTNAME_ROTATION_ANGLE, false, SvOi::SVResetItemToolAndDependent, false);
	RegisterEmbeddedObject( &m_svRotationPointX, SvPb::RotationPointXEId, IDS_OBJECTNAME_ROTATION_POINT_X, false, SvOi::SVResetItemToolAndDependent, false);
	RegisterEmbeddedObject( &m_svRotationPointY, SvPb::RotationPointYEId, IDS_OBJECTNAME_ROTATION_POINT_Y, false, SvOi::SVResetItemToolAndDependent, false);
	RegisterEmbeddedObject( &m_voProfileOrientation, SvPb::ProfileOrientationEId,IDS_OBJECTNAME_LINEARTOOL_ORIENTATION, false, SvOi::SVResetItemToolAndDependent, true);
	RegisterEmbeddedObject( &m_voUseProfileRotation, SvPb::LinearToolUseRotationEId, IDS_OBJECTNAME_LINEAR_TOOL_USE_ROTATION, false, SvOi::SVResetItemToolAndDependent, true);
