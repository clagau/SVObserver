...
namespace SvCmd
{
	...
	public:
		...
		DestroyChildObject(const DestroyChildObject&) = delete;
		DestroyChildObject& operator=(const DestroyChildObject&) = delete;

		//************************************
		/// \param rTaskObjectListID [in] SVGUID of the task-object-list-object.
		/// \param rObjectID [in] SVGUID of the object which should be destroyed.
		/// \param flags [in] Define if after destroying object the default inputs are set and/or inspection are reset.
		//************************************
		DestroyChildObject(const SVGUID& rTaskObjectListID, const SVGUID& rObjectID, FlagEnum flag = Flag_None)
			: m_InstanceID(rTaskObjectListID), m_ObjectID(rObjectID), m_flag(flag) {};

		virtual ~DestroyChildObject() {};
