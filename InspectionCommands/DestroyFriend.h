...
namespace SvCmd
{
	...
	public:
		...
		DestroyFriend(const DestroyFriend&) = delete;
		DestroyFriend& operator=(const DestroyFriend&) = delete;

		/// \param rTaskObjectListID [in] SVGUID of the task-object-list-object.
		/// \param rObjectID [in] SVGUID of the friend which should be destroyed.
		/// \param flags [in] Define if after destroying object the default inputs are set and/or inspection are reset.
		DestroyFriend(const SVGUID& rTaskObjectListID, const SVGUID& rFriendId, FlagEnum flag)
			: m_InstanceID(rTaskObjectListID)
			, m_FriendID(rFriendId)
			, m_flag(flag)
		{};

		virtual ~DestroyFriend() {};
		...
		HRESULT Execute()
		{
			...
			if( nullptr != pTaskObjectList && nullptr != pObject )
			{
				...
				{
					hr = SvStl::Err_10033_DestroyFriendObject_Failed;
				}
			}
			else
			{
				hr = SvStl::Err_10032_DestroyFriendObject_InvalidParameter;
			}
			return hr;
		}
