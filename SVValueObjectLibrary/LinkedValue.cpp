...
bool LinkedValue::setDirectValue(const _variant_t& rValue)
{
	if (SvPb::LinkedValueTypeEnum::TypeDecimal == m_valueType || SvPb::LinkedValueTypeEnum::TypeText == m_valueType
		|| SvPb::LinkedValueTypeEnum::TypeStates == m_valueType)
	{
...
_variant_t LinkedValue::validateValue(const SvPb::LinkedValue& rLinkedValue) const
{
	...
	switch (rLinkedValue.option())
	{
		case SvPb::LinkedSelectedOption::DirectValue:
		{
			if (SvPb::LinkedValueTypeEnum::TypeDecimal == m_valueType || SvPb::LinkedValueTypeEnum::TypeText == m_valueType || SvPb::LinkedValueTypeEnum::TypeStates == m_valueType)
			{
...
bool LinkedValue::UpdateConnection(SvStl::MessageContainerVector* pErrorMessages)
{
	...
	switch (getSelectedOption())
	{
		...
		case SvPb::LinkedSelectedOption::DirectValue:
		{
			...
			if (m_valueType == SvPb::TypeStates)
			{
				Result = UpdateChildrenForDefaultState(pErrorMessages);
...
bool LinkedValue::UpdateChildrenForDefaultState(SvStl::MessageContainerVector* pErrorMessages)
{
	...
	SVObjectClass* pTool = this->GetAncestor(SvPb::SVToolObjectType, false);
	if (!pTool)
	{
		return false;
	}
	...
	for (int i = 0; i < N; i++)
	{
		if (data[i].isIndirectValue)
		{
			...
			if (pValueObject)
			{
				...
				result = resetChild(i, pValueObject, pErrorMessages, createStruct) && result;
			}
			else
			{
				result = false;
...
bool LinkedValue::CheckLinkedObject(const SVObjectClass* const pLinkedObject, const variant_t& rDefault, SvStl::MessageContainerVector* pErrorMessages) const
{
	...
	{
		if (SvPb::SVToolObjectType == pLinkedObject->GetObjectType() && m_valueType == SvPb::TypeStates)
			return true;
