...
namespace Seidenader
{
	namespace  SVXMLLibrary
	{
		...
		HRESULT  SaxXMLHandler<TreeType>::OnEndElement(const wchar_t  *pwchNamespaceUri, int cchNamespaceUri,const wchar_t *pwchName,  int cchName, int depth )
		{
			...
			else if(m_status == PARSING_DATATREE && Elementtype == SaxTreeElement::DATA_Element)
			{

				SaxTreeElement* pSaxTreeElement = GetCurrentSaxTreeElement();
				if(pSaxTreeElement == nullptr ||  Elementtype !=  pSaxTreeElement->GetElementType() )
				{
					ASSERT(false);
					std::string msg = "Invalid Pointer";
					SvStl::MessageMgrNoDisplay Exception(SvStl::LogOnly);
			...
			else if(m_status == PARSING_DATATREE && Elementtype == SaxTreeElement::NODE_Element)
			{	
				if(m_ParseArray)
				{
					///TODO error + bstr 
